...
const bookmarkFilterState = {
  rootParentId: null,
  activeFilterId: BOOKMARK_FILTER_ALL_ID,
  rootItems: [],
  directBookmarks: [],
  childFolders: [],
...
function resetBookmarkFilterState() {
  bookmarkFilterState.rootParentId = null;
  bookmarkFilterState.activeFilterId = BOOKMARK_FILTER_ALL_ID;
  bookmarkFilterState.rootItems = [];
  bookmarkFilterState.directBookmarks = [];
  bookmarkFilterState.childFolders = [];
  bookmarkFilterState.currentViewParentId = null;
  bookmarkFilterState.currentViewItems = [];
  bookmarkFilterState.currentVisibleItemIds = [];
  bookmarkFilterState.requestToken = 0;
}
...
function updateFolderName(bookmarkId) {
  ...
  if (isMainPageBookmarkFilterEnabled()) {
...
function isMainPageBookmarkFilterEnabled() {
...
function createBookmarksEmptyState() {
  const emptyState = document.createElement('div');
  emptyState.className = 'bookmark-empty-state';
  emptyState.textContent = getLocalizedMessage('emptyBookmarksState');
  return emptyState;
}

function removeBookmarkFromFilterState(bookmarkId, parentId) {
  const removeItem = (items = []) => items.filter(item => item.id !== bookmarkId);

  bookmarkFilterState.rootItems = removeItem(bookmarkFilterState.rootItems);
  bookmarkFilterState.directBookmarks = removeItem(bookmarkFilterState.directBookmarks);

  if (bookmarkFilterState.currentViewParentId === parentId) {
    bookmarkFilterState.currentViewItems = removeItem(bookmarkFilterState.currentViewItems);
    bookmarkFilterState.currentVisibleItemIds = bookmarkFilterState.currentVisibleItemIds.filter(id => id !== bookmarkId);
  }
}

function ensureBookmarksEmptyState() {
  const bookmarksList = document.getElementById('bookmarks-list');
  if (!bookmarksList) {
    return;
  }

  const visibleCards = bookmarksList.querySelectorAll('.bookmark-card, .bookmark-folder');
  const hasEmptyState = bookmarksList.querySelector('.bookmark-empty-state');

  if (visibleCards.length === 0 && !hasEmptyState) {
    bookmarksList.appendChild(createBookmarksEmptyState());
  }
}

function updateBookmarkFilterTabSelection() {
  const filterBar = document.getElementById('bookmark-folder-filter-bar');
  if (!filterBar) return;

  filterBar.querySelectorAll('.bookmark-folder-filter-tag').forEach(tag => {
    tag.classList.toggle('active', tag.dataset.filterId === bookmarkFilterState.activeFilterId);
  });
}
...
function renderBookmarkItems(items, parentId, { showFolders = true } = {}) {
  ...
  const sortedItems = getSortedBookmarkItems(items);
  const visibleItems = showFolders ? sortedItems : sortedItems.filter(item => item.url);
  const fragment = document.createDocumentFragment();

  bookmarkFilterState.currentViewParentId = parentId;
  bookmarkFilterState.currentViewItems = sortedItems;
  bookmarkFilterState.currentVisibleItemIds = visibleItems.map(item => item.id);

  if (visibleItems.length === 0) {
    fragment.appendChild(createBookmarksEmptyState());
  } else {
    visibleItems.forEach((item, index) => {
      const element = item.url
        ? createBookmarkCard(item, index)
        : createFolderCard(item, index);
  ...
  bookmarksList.appendChild(fragment);
  bookmarksList.dataset.parentId = parentId;
  bookmarksList.dataset.rootParentId = bookmarkFilterState.rootParentId || parentId;
  bookmarksList.dataset.activeFilterId = bookmarkFilterState.activeFilterId;
...
async function switchBookmarkFilterTab(filterId) {
  if (!isMainPageBookmarkFilterEnabled()) {
    return;
  }

  const requestToken = ++bookmarkFilterState.requestToken;
  const bookmarksContainer = document.querySelector('.bookmarks-container');
  bookmarkFilterState.activeFilterId = filterId;
  updateBookmarkFilterTabSelection();

  if (filterId === BOOKMARK_FILTER_ALL_ID) {
    bookmarksContainer?.classList.remove('loading');
    renderBookmarkItems(bookmarkFilterState.rootItems, bookmarkFilterState.rootParentId, { showFolders: false });
    return;
  }

  const cached = bookmarksCache.get(filterId);
  if (cached) {
    bookmarksContainer?.classList.remove('loading');
    renderBookmarkItems(cached.bookmarks, filterId, { showFolders: false });
    return;
  }
  ...
  try {
    const folderChildren = await new Promise((resolve, reject) => {
      chrome.bookmarks.getChildren(filterId, (children) => {
        if (chrome.runtime.lastError) {
          reject(chrome.runtime.lastError);
          return;
        }
    ...
    if (bookmarkFilterState.requestToken !== requestToken || bookmarkFilterState.activeFilterId !== filterId) {
      return;
    }

    bookmarksCache.set(filterId, folderChildren);
    renderBookmarkItems(folderChildren, filterId, { showFolders: false });
  } catch (error) {
    console.error('Error loading filter tab bookmarks:', error);

    if (bookmarkFilterState.requestToken === requestToken && bookmarkFilterState.activeFilterId === filterId) {
      renderBookmarkItems([], filterId, { showFolders: false });
    }
  } finally {
    if (bookmarkFilterState.requestToken === requestToken) {
      bookmarksContainer?.classList.remove('loading');
    }
  }
}

function renderBookmarkFilterTabs() {
  const filterBar = document.getElementById('bookmark-folder-filter-bar');
  if (!filterBar) {
    return;
  }
  ...
  const tabDefinitions = [
    {
      id: BOOKMARK_FILTER_ALL_ID,
      title: getLocalizedMessage('allBookmarksTab')
    },
    ...bookmarkFilterState.childFolders.map(folder => ({
      id: folder.id,
      title: folder.title
    }))
  ];
  ...
  tabDefinitions.forEach(tab => {
    ...
    tag.type = 'button';
    tag.className = 'bookmark-folder-filter-tag';
    tag.dataset.filterId = tab.id;
    tag.textContent = tab.title;
...
function initializeMainPageBookmarkFilters(bookmark) {
  const sortedItems = getSortedBookmarkItems(bookmark.children || []);

  bookmarkFilterState.rootParentId = bookmark.id;
  bookmarkFilterState.activeFilterId = BOOKMARK_FILTER_ALL_ID;
  bookmarkFilterState.rootItems = sortedItems;
  bookmarkFilterState.directBookmarks = sortedItems.filter(item => item.url);
  bookmarkFilterState.childFolders = sortedItems.filter(item => !item.url);
...
function translateFilteredDropIndex(bookmarksList, movedItemId, fallbackIndex) {
  if (!bookmarksList || bookmarkFilterState.currentViewItems.length === 0) {
    return fallbackIndex;
  }

  const renderedItemIds = Array.from(bookmarksList.children)
    .map(element => element.dataset.id)
    .filter(Boolean);

  if (renderedItemIds.length === 0) {
    return fallbackIndex;
  }

  const allItems = getSortedBookmarkItems(bookmarkFilterState.currentViewItems);
  const hasHiddenFolders = allItems.some(item => !item.url);
  ...
  let visibleCursor = 0;
  const mergedOrder = allItems.map(item => {
    if (!item.url) {
      return item.id;
    }

    return renderedItemIds[visibleCursor++];
  });

  const translatedIndex = mergedOrder.indexOf(movedItemId);
  return translatedIndex === -1 ? fallbackIndex : translatedIndex;
}
...
function displayBookmarks(bookmark) {
  ...
  if (isMainPageBookmarkFilterEnabled()) {
    initializeMainPageBookmarkFilters(bookmark);
    return;
  }

  resetBookmarkFilterState();
  renderBookmarkItems(bookmark.children || [], bookmark.id, { showFolders: true });
}
...
function deleteBookmark(bookmarkId, bookmarkTitle) {
  ...
  chrome.bookmarks.remove(bookmarkId, function() {
    if (chrome.runtime.lastError) {
      ...
      removeBookmarkFromFilterState(bookmarkId, parentId);
      ...
      setTimeout(ensureBookmarksEmptyState, 320);
    }
  });
}
  ...
  if (bookmarksList) {
    new Sortable(bookmarksList, {
      ...
      onEnd: function (evt) {
        ...
        const newIndex = translateFilteredDropIndex(bookmarksList, itemId, evt.newIndex);
